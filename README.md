...
**Custom ontology: model your agent's world _your way_.** By default, Graphiti extracts generic entities and relationships — and that works fine. But when you define custom entity types with attributes and typed relationships, you're telling the extraction LLM exactly what to look for in your domain. A `Project` with a `status` enum. A `Technology` with a `category`. A `Uses` relationship connecting them. Your agent natively understands the language of your domain — or your life. Recent research confirms the value of this approach: [Apple's ODKE+ system](https://arxiv.org/abs/2509.04696) (Khorshidi et al., 2025) demonstrated that ontology-guided extraction achieves 98.8% precision across 19 million facts — up from 91% with raw LLM extraction — by dynamically generating ontology snippets tailored to each entity type. A [comparative study on ontology-guided KG construction and RAG performance](https://arxiv.org/abs/2511.05991) (2025) found that ontology-guided knowledge graphs substantially outperform vector retrieval baselines, with the added benefit that a well-defined ontology is a one-time investment that reduces ongoing LLM costs. *Fighting words*: Generic extraction is a blunt instrument. When every entity is just a "thing" and every relationship is `RELATES_TO`, your graph is a soup of ambiguity. Custom ontology turns it into a structured model of _your_ world — the agent doesn't just remember facts, it understands them in context.
