...
Tools don't receive agent context (OpenClaw calls `execute(toolCallId, params)` — no ctx). The `before_prompt_build` hook captures `ctx.agentId` via `setGroupId`, tools read via `getGroupId`. Group ID resolution (`agentId ?? "default"`) is inlined at call sites in hooks.
...
| unified-search | `memory_search` combines native Markdown + graph facts in parallel. When results are found, appends the interpretation instruction (same as auto-recall) but not the further querying instruction. Native results with empty `results: []` metadata JSON are filtered out (`countNativeResults()` in `src/hooks.ts`). Same filter used by auto-recall. |
...
| cli-diagnostics | `gralkor status/check/search/clear` under `openclaw plugins`; group ID required for search/clear |
