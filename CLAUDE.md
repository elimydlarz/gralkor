...
- **`api.on(event, handler)`** — Registers a hook handler. Alternatively, `registerHook(event, handler, metadata)` takes a third `metadata` arg with `{ name: string }` — the gateway does `metadata.name.trim()` so omitting it crashes. Our code uses `api.on` (no metadata needed). **Critical: hook handlers receive a single `ctx` object** with `{ agentId?, userMessage?, agentResponse? }` — **not** `(event, ctx)`. Using two parameters silently breaks because `ctx` is `undefined`.
