...
1. **Sync Python environment** — runs `uv sync --no-dev --frozen --directory {serverDir}` with `UV_PROJECT_ENVIRONMENT={dataDir}/venv`. If a `wheels/` directory exists in `serverDir`, sets `UV_FIND_LINKS` so bundled wheels (e.g. the falkordblite arm64 wheel from `make pack`) are preferred over PyPI. uv handles Python discovery/installation, venv creation, and dependency resolution from `pyproject.toml` + `uv.lock`.
...
- `falkordblite` 0.9.0 has an sdist packaging bug: the sdist is built on x86-64 CI and bundles `falkordb-x64.so` as `falkordb.so`. On aarch64 hosts where the `manylinux_2_39` wheel is incompatible (glibc < 2.39, e.g. Debian 12 with glibc 2.36), pip/uv builds from sdist. The `download_falkordb_module()` in `setup.py` skips download because `falkordb.so` already exists in the sdist, resulting in an x86-64 binary on an aarch64 host. Redis-server then fails to load the module (`dlopen()` fails), causing `RedisLiteServerStartError`. The secondary `AttributeError: 'Redis' object has no attribute 'connection_pool'` is a cleanup artifact — `redis.Redis.__init__()` was never called because `_start_redis()` failed first. The `_log_falkordblite_diagnostics()` function in `server/main.py` logs architecture info to help diagnose this. **Workaround (implemented):** `scripts/pack.sh` builds a correct falkordblite wheel for linux/arm64 via Docker, retags it from `cp313-cp313` to `py3-none` (the binaries are standalone executables loaded via subprocess/dlopen, not Python C extensions, so they're Python-version-independent), and bundles it in the tarball under `server/wheels/`. The server manager sets `UV_FIND_LINKS=server/wheels` so the bundled wheel is preferred over the broken PyPI sdist. Upstream fix: exclude `falkordb.so` from sdist and/or build wheels targeting older glibc (manylinux_2_28).
