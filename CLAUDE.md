...
- `Gralkor.Client.HTTP` — Req adapter. Reads `Application.get_env(:gralkor_ex, :client_http)` with `:url` (required) and `:plug` (optional `Req.Test` plug). No Authorization header. No retries at this layer (`retry: false`) — non-2xx responses and transport errors surface immediately; the google-genai SDK at L6.5 is the only retrier in the stack (see `TEST_TREES.md` > Retry ownership). Per-endpoint receive_timeouts (health 2s, recall 12s aligning with the server's 10s deadline + 2s transport margin, capture/end_session 5s, memory_search 30s, memory_add 60s), tuple→list recursion before Jason encoding, blank-session_id raises `ArgumentError` on recall/capture/memory_search/end_session, `{:error, {:http_status, status, body}}` on non-2xx.
