...
- `Gralkor.Server` — GenServer. `init/1` is non-blocking; `handle_continue(:boot)` writes config, **reaps any stale Python child** via `$GRALKOR_DATA_DIR/server.pid` (SIGTERM → wait → SIGKILL), **pre-flights the bind port** (`:gen_tcp.listen` on the port parsed from `server_url`) — if already bound it stops with `{:boot_failed, :port_in_use}` before spawning — then spawns `uv run uvicorn main:app --host 127.0.0.1 --port 4000 --timeout-graceful-shutdown 30` via `Port.open`, writes the new OS pid to `server.pid`, health-polls at 500ms up to a configurable `boot_timeout_ms` (default 120s), then schedules a 60s monitor. Boot loop peeks the mailbox each iteration so a dying port fails fast instead of hitting the full timeout. Two safety nets because BEAM SIGKILL / Ctrl-C abort skips `terminate/2`, orphaning the Python child: the pid-file reap covers pids we previously wrote; the port pre-flight covers anything else (true orphans we didn't record, or unrelated listeners) — without them the next boot would crash-loop on `EADDRINUSE`. On graceful shutdown `terminate/2` kills the child and removes `server.pid`.
