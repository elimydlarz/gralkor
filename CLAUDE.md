...
2. `extractUserMessageFromPrompt()` strips leading `System:` event lines, then strips any `<label> (untrusted metadata):\n```json\n...\n``` ` wrapper from `event.prompt` (the label varies — gateway currently sends `Sender`, previously `Conversation info`), skips system prompts.
...
| Prompt parsing robustness | Auto-recall correctly detects system prompts even when queued events (e.g. Telegram reactions) are prepended to `ctx.prompt`. Metadata wrapper stripping uses a flexible regex (`/^.+?\(untrusted metadata\):/`) so it works regardless of the label the gateway uses (e.g. `Sender`, `Conversation info`). |
