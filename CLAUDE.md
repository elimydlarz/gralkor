...
- `Gralkor.OrphanReaper` — pre-OTP cleanup. `reap/1` (accepts `shell:` injection for tests) shells `lsof` for port 4000, SIGKILLs if command line contains `gralkor_ex/priv/server` (the packaged server path under `:code.priv_dir(:gralkor_ex)`), raises with foreign command line otherwise.
