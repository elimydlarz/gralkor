...
An OpenClaw plugin giving AI agents persistent, temporally-aware memory via knowledge graphs. Uses Graphiti backed by FalkorDB.

A memory plugin (`kind: "memory"`) replacing native `memory-core`. Auto-recall searches both graph and native Markdown before each turn; auto-capture buffers session messages and flushes one episode per session.
...
`createMemorySearchTool()` uses `MemoryIndexManager` (SQLite FTS5 + optional vector embeddings). **Known bug:** FTS-only mode (no embedding key) → `syncMemoryFiles()` returns early → `memory_search` always empty. Workaround: configure an embedding provider.
...
- TypeScript, ES modules, target ES2022, bundler module resolution
...
- Native `execute()` returns `{ content: [{ type: "text", text }] }`, not string. Use `unwrapToolResult()`.
- `falkordblite` installs as Python module `redislite`, not `falkordblite`
- `falkordblite` 0.9.0 sdist bundles x86-64; aarch64 + glibc < 2.39 → `RedisLiteServerStartError`. Fix: bundled arm64 wheel via `make pack`.
