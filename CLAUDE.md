...
- `falkordblite` (via `redislite`) is incompatible with `redis-py` 7.x (`self.connection_pool` was removed/changed). The `redis<7` pin in `requirements.txt` and `pyproject.toml` prevents this. `falkordblite` itself supports Python 3.12–3.14 (wheels exist for all three).
