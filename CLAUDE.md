...
Service `gralkor-server` (`src/server-manager.ts`): `uv sync --no-dev --frozen` (venv in `dataDir`), force-install bundled wheels, write `config.yaml`, pass API keys as env vars (built synchronously from `config.*ApiKey` strings by `buildSecretEnv()` in `register.ts`). Before spawning: pre-flight `GET /health` — if response is 200 with gralkor signature (`status` + `graph` keys present), skip spawn and return (orphaned server from prior process). If port is occupied by an unrelated service, proceeds with spawn (will fail with EADDRINUSE). Otherwise: spawn uvicorn on `127.0.0.1:8001` with `CONFIG_PATH`/`FALKORDB_DATA_DIR`/resolved API keys. No `FALKORDB_URI` → embedded FalkorDBLite. Poll `/health` 500ms (120s timeout), monitor 60s. On healthy: `serverReady.resolve()` (module-level). SIGTERM → 5s → SIGKILL. First start ~1-2 min.
