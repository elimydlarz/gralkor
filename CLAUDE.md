...
Tools don't receive ctx (`execute(toolCallId, params)`). `before_prompt_build` stores `groupId` in a `Map<sessionKey, groupId>` (`groupIdBySession`) and injects the `sessionKey` into the `<gralkor-memory>` block. Tools require `session_key` — the model reads it from the memory block and passes it back, allowing tools to call `getGroupId(sessionKey)` for a race-free lookup. `getGroupId` returns `undefined` for unregistered keys; tools throw rather than route to a wrong partition. Sanitization (`sanitizeGroupId` — hyphens → underscores) happens **once** at write time inside `setSessionData`; all readers (`before_prompt_build` search, flush, tools) get the pre-sanitized value from the map. Auto-recall and flush fall back to `sanitizeGroupId(agentId)` only when `getGroupId` is not provided (test isolation without the full map infrastructure).
...
publish-version-integrity
  when publish succeeds
    ...
    and a git commit and tag are created for the new version (push manually)
