...
- **Auto-capture** (`agent_end` → `DebouncedFlush` keyed by `sessionKey || agentId || "default"` → `session_end` force-flush): `extractMessagesFromCtx` cleans messages, `formatTranscript(messages, llmClient)` (in `src/distill.ts`) groups thinking/`tool_use`/`tool_result` per turn and, for each turn that has any, builds a distill input containing the user message, the behaviour blocks, and the agent's eventual response. The behaviour blocks are the source of truth for what to distill (including dead ends and rejected approaches); the user message and response are passed only as disambiguating context so the LLM can recognise what the actions were about (e.g. that `BOOTSTRAP.md` is a workspace file, not the CSS framework) and never as a filter that erases actions absent from the response. Result is distilled into a first-person `(behaviour: …)` line, then `client.ingestEpisode({ episode_body })`.
...
behaviour-distillation
  formatTranscript (plugin-side, src/distill.ts)
    ...
    when turn has behaviour blocks and llmClient available
      ...
      and the distill input includes the user message and the agent's response alongside the behaviour blocks as disambiguating context
      and the system prompt declares the actions the source of truth for what to distill
      and the system prompt instructs the LLM to capture dead ends and rejected approaches that did not make it into the response
      and the system prompt instructs the LLM to use the response only as disambiguating context, never as a filter that erases actions absent from it
      and the system prompt instructs the LLM not to invent content absent from the inputs
