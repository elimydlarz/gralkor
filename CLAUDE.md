...
5. `flushSessionBuffer()` calls `extractMessagesFromCtx()` which returns `{ episodeBody, thinkingPerTurn }`. For user messages: joins all `text`/`output_text` blocks, then `cleanUserMessageText()` strips system noise — session-start instructions (messages starting with `"A new session was started"` are dropped entirely), metadata wrappers (`Xxx (untrusted metadata):\n```json\n...\n```\n\n` — multiple blocks supported), and `<gralkor-memory>` XML. For assistant messages: `text`/`output_text` → `Assistant: {text}` in `episodeBody`, `thinking` → collected into `thinkingPerTurn` (one entry per agent turn, concatenated with `\n---\n` separator), tool-related blocks (`toolCall`/`toolUse`/`functionCall`) skipped. **Silently drops media** (images, video).
6. Skip if disabled or empty (no text extracted).
7. POST to `/episodes` with `episode_body` (conversation text), `thinking_blocks` (raw thinking per turn), and `reference_time` set to wall-clock time.
8. **Server-side thinking distillation:** The server's `POST /episodes` handler distills each thinking block into a single-sentence action summary via the configured LLM (`_distill_thinking`), then injects `Assistant: (action: {summary})` lines before each turn's first assistant text (`_inject_action_summaries`). If distillation fails for a turn, the action line is silently dropped. This transforms raw operational thinking ("Let me search for...") into semantic action descriptions ("Investigated and resolved the null pointer issue").
9. Buffer is deleted before the API call (so errors don't leave stale entries).
10. `addEpisode` is retried up to 3 times with exponential backoff (1s/2s/4s) for transient errors (network, 5xx, `AbortError`). 4xx client errors are not retried. After exhaustion, the last error propagates to callers.
...
3. Write dynamic `config.yaml` to `dataDir` from plugin settings (`llm`/`embedder` with defaults, plus `ontology` if configured). If ontology is present, server builds Pydantic models at startup via `_build_ontology()` and passes them to every `graphiti.add_episode()` call. Spawn `{venvPython} -m uvicorn main:app --host 127.0.0.1 --port 8001 --no-access-log`. Passes env vars (`CONFIG_PATH` pointing to generated config, `FALKORDB_DATA_DIR`, LLM API keys). Does NOT set `FALKORDB_URI` (absence triggers embedded FalkorDBLite).
...
| thinking-distillation | Thinking blocks collected per turn, sent to server as `thinking_blocks`; server distills each into a single `(action: ...)` summary via LLM before Graphiti ingestion. Failures silently dropped. |
