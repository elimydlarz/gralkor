...
| unified-search | `memory_search` combines native Markdown + graph facts in parallel. Native results with empty `results: []` metadata JSON are filtered out (`countNativeResults()` in `src/hooks.ts`). Same filter used by auto-recall. |
