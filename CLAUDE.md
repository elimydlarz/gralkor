...
| CLI diagnostics | `openclaw plugins gralkor status`, `openclaw plugins gralkor search <group_id> <query...>`, `openclaw plugins gralkor clear <group_id>` available for troubleshooting. Group ID (agent ID) is always required — there is no default. |
...
| Observability | Hooks and tools log `[gralkor]`-prefixed messages: hook-fired events (structural metadata only), result counts, skip reasons, errors. Message bodies and user content are never logged. Uvicorn access logs are disabled via `--no-access-log` to suppress noisy health check polling; application-level logging covers all routes. |
...
| Prompt parsing robustness | Auto-recall strips `System:` event lines, session-start lines, and metadata wrappers in sequence — the first user message after `/new` is correctly extracted even when the gateway combines "A new session was started..." with the user's message in a single prompt. Metadata wrapper stripping uses a flexible regex (`/^.+?\(untrusted metadata\):/`) so it works regardless of the label the gateway uses (e.g. `Sender`, `Conversation info`). When prompt extraction yields nothing (e.g. prompt is only a metadata wrapper with no appended user text), falls back to extracting the last user message from `event.messages`. |
| Query sanitization | Server-side `_sanitize_query()` strips backticks from search queries before passing to graphiti-core, preventing RediSearch syntax errors (graphiti's `_SEPARATOR_MAP` omits backtick) |
...
│   ├── .python-version               # pins Python 3.13 for dev
│   ├── Dockerfile
...
│       ├── test_lifespan.py          # Embedded vs TCP mode switching in lifespan()
│       └── test_integration.py       # Real FalkorDBLite: binary import, DB create, Cypher read/write
│
...
Each agent gets its own graph partition automatically — no configuration needed. Tools don't receive agent context (OpenClaw calls `execute(toolCallId, params)` — no ctx), so the `before_agent_start` hook captures the agent ID from `ctx.agentId` via a `setGroupId` callback, and tools read it via a `getGroupId` closure. Falls back to `"default"` if `agentId` is absent.
...
- If the server **fails to start** (uv not found, uv sync fails, health timeout): the error is caught and logged by `registerServerService`; the service does not throw, so the plugin still loads. Tools/hooks then encounter Graphiti as unreachable.
...
The `files` field in `resources/memory/package.json` controls what goes into the tarball: `dist/`, `server/Dockerfile`, `server/main.py`, `server/pyproject.toml`, `server/uv.lock`, `server/requirements.txt`, `server/wheels` (falkordblite arm64 wheel), `openclaw.plugin.json`, `docker-compose.yml`, `config.yaml`, `.env.example`. The manifest (`openclaw.plugin.json`) is stamped by `scripts/pack.sh` before packing.

**Default (self-managing):** No Docker needed. The plugin's server manager spawns the Graphiti server automatically using embedded FalkorDBLite. Requires `uv` on the deployment host.
...
- The `falkordblite` PyPI package installs its Python module as `redislite`, not `falkordblite`. The correct import is `from redislite.async_falkordb_client import AsyncFalkorDB` (the documented path). The `falkordblite` name is only the package distribution name.
- `falkordblite` 0.9.0 has an sdist packaging bug: the sdist is built on x86-64 CI and bundles `falkordb-x64.so` as `falkordb.so`. On aarch64 hosts where the `manylinux_2_39` wheel is incompatible (glibc < 2.39, e.g. Debian 12 with glibc 2.36), uv builds from sdist. The `download_falkordb_module()` in `setup.py` skips download because `falkordb.so` already exists in the sdist, resulting in an x86-64 binary on an aarch64 host. Redis-server then fails to load the module (`dlopen()` fails), causing `RedisLiteServerStartError`. The secondary `AttributeError: 'Redis' object has no attribute 'connection_pool'` is a cleanup artifact — `redis.Redis.__init__()` was never called because `_start_redis()` failed first. The `_log_falkordblite_diagnostics()` function in `server/main.py` logs architecture info to help diagnose this. **Workaround (implemented):** `scripts/pack.sh` builds a correct falkordblite wheel for linux/arm64 via Docker, retags it to `py3-none-manylinux_2_36_aarch64` (the binaries are standalone executables loaded via subprocess/dlopen, not Python C extensions, so they're Python-version-independent), and bundles it in the tarball under `server/wheels/`. After `uv sync --frozen`, the server manager force-installs bundled wheels via `uv pip install --reinstall --no-deps` — this bypasses the lockfile entirely (`UV_FIND_LINKS` doesn't work with `uv sync --frozen` because lockfile hash verification rejects locally-built wheels). If the wheel isn't compatible with the current platform (e.g. on macOS dev), the error is caught and the PyPI version is kept. Upstream fix: exclude `falkordb.so` from sdist and/or build wheels targeting older glibc (manylinux_2_28).
- The server manager requires `uv` on the system PATH. If `uv` is not found, server startup fails (gracefully — plugin still loads but tools see Graphiti as unreachable). uv handles Python discovery and installation automatically via the `pyproject.toml` `requires-python` constraint.
- First plugin start is slow (~1-2 min) because the server manager runs `uv sync` to create the Python environment and install dependencies. Subsequent starts are fast (venv reused, `uv sync --frozen` is a no-op when the lockfile is satisfied).
...
- Auto-capture errors propagate to the gateway (not swallowed) — this is intentional so failures are visible. The gateway runs `agent_end` fire-and-forget (not awaited, no `AbortSignal` passed), so errors are caught by a `.catch()` and logged but don't block the gateway. Despite no abort signal being passed, `AbortError` has been observed in production — likely from Node's HTTP layer (connection reset, process SIGTERM during fetch), not from the gateway aborting the hook
...
- Auto-recall strips leading `System: ...` event lines and session-start lines (`"A new session was started..."`) from `ctx.prompt` before extracting the user message. The gateway may combine session-start with the user's first message in a single prompt (e.g. `/new` followed by "Hello") — the stripping is sequential so the user message is preserved
