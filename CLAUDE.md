...
`pnpm run publish:all -- patch|minor|major` bumps, builds, publishes npm + ClawHub, commits and tags. `publish:npm` / `publish:clawhub` for one-at-a-time (each accepts `current` to skip the bump). `publish:ex` releases the Elixir adapter (`ex/mix.exs`) on its own Hex version stream, tagged `ex-v${version}`. `publish:ts` releases the TypeScript adapter (`ts/package.json`) on its own npm version stream, tagged `gralkor-ts-v${version}`. Neither is included in `publish:all` — each adapter's release cadence is independent of the OpenClaw plugin's. `pnpm run pack` builds a deployment tarball (arm64 wheel via Docker). Requires `uv`. Docker HOME split: `ln -sfn /data/.openclaw /root/.openclaw`. Behaviour in the publish test trees.
