...
- `falkordblite` (via `redislite`) depends on `redis-py` with no version constraint. The `uv.lock` resolves to redis-py 7.x. `falkordblite` itself supports Python 3.12–3.14 (wheels exist for all three).
