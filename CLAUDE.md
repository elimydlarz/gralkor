...
- **BUG: Auto-capture feedback loop** — The gateway injects auto-recall's `prependContext` as a user message in `ctx.messages`. `extractMessagesFromCtx()` doesn't strip the `<gralkor-memory>` XML, so the full recalled facts/entities get stored back into the graph as a new episode. Graphiti extracts entities from this meta-content, polluting the graph with internal details. Fix: strip `<gralkor-memory>...</gralkor-memory>` blocks from user text in `extractMessagesFromCtx()`.
- **BUG: Auto-recall prompt parsing** — `extractUserMessageFromPrompt()` uses `startsWith("A new session was started")` to detect system prompts. When queued events are prepended (e.g. `System: [...] Telegram reaction added: ...`), the prompt no longer starts with that string, so the entire prompt (event + session instruction) is sent as a search query. Fix: use a more robust check (regex, `includes`, or strip `System:` lines first).
