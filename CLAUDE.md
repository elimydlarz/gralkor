...
- **Behaviour distillation turn alignment bug:** `_format_transcript()` only appends to `turns_thinking` when `current_thinking` is non-empty, so turns without behaviour blocks (no thinking, tool_use, or tool_result) are skipped. The injection loop maps `summaries[0]` → turn 0, `summaries[1]` → turn 1, etc. When early turns have no behaviour (simple text replies), summaries from later turns get injected at the wrong positions. More visible with tool use since early turns commonly lack behaviour blocks.
