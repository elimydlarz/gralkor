...
- **`Gralkor.Client.HTTP`** — Req-based adapter. Reads `:gralkor_ex, :client_http` (keys: `:url` required, `:plug` optional `Req.Test` plug for stubbing). No auth, retry-once on transient transport errors (`:closed`/`:timeout`/`:econnreset`) — HTTP responses and other transport failures surface immediately — per-endpoint `receive_timeout`s calibrated to workload (2s `/health`, 5s `/recall`/`/capture`/`/session_end`, 10s `/tools/memory_search`, 60s `/tools/memory_add`). Normalises Elixir tuples to lists before Jason encodes (so `{:ok, _}` tool results in capture event traces don't crash).
