...
if [ -n "$API_KEY" ]; then
  ...
  openclaw config set plugins.entries.gralkor.config.googleApiKey "$API_KEY" 2>&1
else
...
openclaw config get plugins 2>&1 | sed -E 's/("googleApiKey"\s*:\s*")[^"]+/\1***REDACTED***/g' || true
echo ""
...
timeout 180 openclaw gateway start --workspace /tmp/harness-workspace 2>&1 || EXIT_CODE=$?
